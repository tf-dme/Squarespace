# Nutzung des Powershell Module Microsoft.Graph.Groups
Update-MgGroup -GroupId '5a13db6f-1786-4b3c-b88c-d8b148a99e9a' -Bodyparameter @{'thinformatics_CustomAttributes1'={'PIMEnabledGroup' = $false}}

# Direkte Nutzung der REST API
$Query = @{
    Method = "PATCH"
    Headers = @{
        Authorization = ("Bearer " + $JWT)
    }
    URI = "https://graph.microsoft.com/v1.0/groups/5a13db6f-1786-4b3c-b88c-d8b148a99e9a"
    ContentType = "application/json"
    Body = @{
        thinformatics_CustomAttributes1 = @{
            PIMEnabledGroup = $true
        }
    } | ConvertTo-Json
}
Invoke-RestMethod @Query